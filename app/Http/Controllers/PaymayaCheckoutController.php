...
//        $grand_total = $total + $service_charges
        ...
        session(['id' => $itemCheckout->id, 'bills' => $itemArr]);
...
//                    return redirect()->to('/');
                    ...
                    try
                    {
                        ...
                        $total = $checkout['totalAmount']['details']['subtotal'];
                        ...
                        foreach($checkout['items'] as $item)
                        {
                            ...
                            $validateBillNum = DB::connection('pgsql_2')->table('colhdr')->where('bill_num', $item['name'])->count();
                            ...
                            if(!$validateBillNum)
                            {
                                ...
                                $billing_info->push(['bill_num' => $item['name'], 'amount' => $item['amount']['value']]);
                            }
                        }
                    }
                    catch(\Illuminate\Database\QueryException $ex) {
                        DB::rollBack();
                        Session::flash('error', 'Transaction Failed Something went wrong. Payment was not procceeded');
                        return redirect()->to('/');
                    }
